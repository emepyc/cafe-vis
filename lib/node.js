...
var iterator = require ("./utils.js").iterator;
...
// 	    }
	...
	var new_children = [];
	for (var i=0; i<data.children.length; i++) {
	    new_children.push(_node(data.children[i]));
	}
	...
	for (var i=0; i<data.children.length; i++) {
	    _node(data.children[i]).sort(cbak);
	}
    });
	...
	if (data.children !== undefined) {
		...
		console.log("<<<<");
		console.log(_node(data.children[i]));
		n.apply(cbak);
	    }
	}
    });
...
//     };
	...
	var children = [];
	for (var i=0; i<data.children.length; i++) {
	    children.push(_node(data.children[i]));
	}
	return children;
    });
